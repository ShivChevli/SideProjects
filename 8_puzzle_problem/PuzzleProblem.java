...
class PuzzleProblem {
    ...
    static Node aStar(int start[][], int end[][]) {
        ...
        // System.out.println("======================================================================");
        // System.out.println("======================================================================");
        int approxCost = getHuristic(head, end);
        head.setHuristicCost(approxCost);
        if (approxCost == 0) {
            return head;
        }
        count++;
        ...
        while (!childQueue.isEmpty()) {
            ...
            if (zeroY == 2) {
                ...
                Node child = moveX(((ExtraData) tmp.data).state, newState, zeroX, zeroY, tmp,
                        true, count);
                int tmpHurustic = getDistention(newState, end);
                // System.out.println("Parent :- " + tmp.name);
                // System.out.println("child :- " + child.name);
                // printBoard(((ExtraData) child.data).state);
                // System.out.println("######################################");
                if (tmpHurustic == 0) {
                    return child;
                }
    ...
    /*
     * calculate Huristic Value for state
     */
    static int getHuristic(Node child, int[][] endNode) {
...
// Genral graph structure - Create State Space Tree Graph Structure to track
// Nodes
class Node {
    public char name;
    Node parent;
...
// Comparator Class to Impliment Prority Quea
class The_Comparator implements Comparator<Node> {

    @Override
    public int compare(Node o1, Node o2) {
        // TODO Auto-generated method stub
        int cost1 = o1.getHuristicCost();
        int cost2 = o2.getHuristicCost();
        if (cost1 == cost2) {
            return 0;
        } else if (cost1 > cost2) {
            return 1;
